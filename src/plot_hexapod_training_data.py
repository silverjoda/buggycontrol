...
def process_and_plot(param, rew, param_rew):
    param_df = pd.DataFrame(param)
    rew_df = pd.DataFrame(rew)
    param_rew_df = pd.DataFrame(param_rew)
...
# Do PCA
process_and_plot(e2e_param_vec, e2e_rew_vec, e2e_param_rew_vec)
process_and_plot(lsa_param_vec, lsa_rew_vec, lsa_param_rew_vec)
plt.show()